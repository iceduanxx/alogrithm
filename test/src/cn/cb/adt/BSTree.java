...
public class BSTree extends BinaryTreeLinked implements SearchTable {
     ...
     * @param find
     */
    public void pathTree(int find) {
        LinkedList list = new LinkedListDLNode();
        recursionPathTree(list, root, find);
    }

    /**
     * 这里的list当做Stack,之所以不用S是因为stack里面的元素只能取出来遍历
     *
     * @param list
     * @param binTreeNode
     * @param findNum
     */
    public void recursionPathTree(LinkedList list, BinTreeNode binTreeNode, int findNum) {
        //边界条件
        if (binTreeNode == null || findNum <= 0) return;
        list.insertLast(binTreeNode);
        //判断是否为叶节点
        if (binTreeNode.getLChild() == null && binTreeNode.getRChild() == null) {
            int sum = 0;
            Iterator iterator = list.elements();
            for (iterator.first(); !iterator.isDone(); iterator.next()) {
                sum += (Integer) ((BinTreeNode) (((Node) iterator.currentItem()).getData())).getData();
            }
            if (sum == findNum) {
                for (iterator.first(); !iterator.isDone(); iterator.next()) {
                    System.out.print(((BinTreeNode) (((Node) iterator.currentItem()).getData())).getData() + " ");
                }
        ...
        recursionPathTree(list, binTreeNode.getLChild(), findNum);
        recursionPathTree(list, binTreeNode.getRChild(), findNum);
    }
}
